...
/**
 * This class demonstrates how to use the Java 11+ HttpClient with Avro serialization to produce messages over REST.
 ...
 */
public class AvroRestPublisherHttpClient implements AutoCloseable {
     ...
     */
    public AvroRestPublisherHttpClient(String brokerHost, int port) {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        if (args.length < 2) {
            System.out.printf("Usage: %s <host> <port> %n", AvroRestPublisherHttpClient.class.getName());
            System.exit(-1);
        }
        ...
        try (AvroRestPublisherHttpClient publisher = new AvroRestPublisherHttpClient(host, portNumber)) {
            ...
            try (Serializer<User> serializer = new AvroSerializer<>()) {
                ...
                while (!exitListener.isDone()) {
                    publisher.publishToTopic(serializer, TOPIC, index);
                    index++;
     ...
     */
    public void publishToTopic(Serializer<User> serializer, String topic, int id) {
