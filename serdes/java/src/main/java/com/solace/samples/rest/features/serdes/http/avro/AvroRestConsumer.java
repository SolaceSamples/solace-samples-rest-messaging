...
/**
 * This class demonstrates how to use the Java HttpServer with Avro Schema deserialization to consume messages over REST.
 * It deserializes messages using the {@link AvroDeserializer} and consumes it from a topic on a Solace message broker using an HTTP POST request.
 */
public class AvroRestConsumer {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        if (args.length < 1) {
            ...
            System.out.printf("Usage: %s <post-request-target> [<port>] [<http-topic-header-key>] %n", AvroRestConsumer.class.getName());
            System.exit(-1);
        }
        ...
        try (Deserializer<GenericRecord> deserializer = new AvroDeserializer<>()) {
            ...
            server.createContext(postRequestTarget, new SerdesMessageHandler<GenericRecord>(deserializer, Object::toString, httpTopicHeaderKey));
    ...
    static class SerdesMessageHandler<T> implements HttpHandler {
        ...
        final Function<T, String> recordToStringFunc;

        /**
         * Constructor for the SerdesMessageHandler.
         * @param deserializer the deserializer to use
         * @param recordToString a function to convert the record to a string
         * @param httpTopicHeaderKey the HTTP header key for the topic
         */
        public SerdesMessageHandler(Deserializer<T> deserializer, Function<T, String> recordToString, String httpTopicHeaderKey) {
            this.deserializer = deserializer;
            this.recordToStringFunc = recordToString;
         ...
         * <a href="https://docs.solace.com/API/RESTMessagingPrtl/Solace-REST-Message-Encoding.htm?Highlight=REST%20custom%20headers#solace-message-custom-properties">Solace Message Custom Properties</a></li>
        ...
        public void handleSerdesMessage(HttpExchange exchange) throws IOException {
            ...
            T record = deserializer.deserialize(optionalTopic.orElse(""), messagePayload, serdesHeaders);
            ...
            System.out.println("Body: " + recordToStringFunc.apply(record));
            System.out.printf("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -%n");
        }

        /**
         * Handles the HTTP request. For more details on configuring http status codes, see:
         * <a href="https://docs.solace.com/Services/Managing-RDPs.htm#configuring-http-status-codes">Configuring Http Status Codes</a></li>
         * @param exchange the HTTP exchange
         * @throws IOException if an I/O error occurs
         */
        @Override
